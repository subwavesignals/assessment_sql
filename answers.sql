...
SELECT m.name AS model_name, b.name AS brand_name, headquarters FROM models AS m JOIN brands AS b ON m.brand_id=b.brand_id WHERE 
b.name='Ford' AND m.name='Mustang';
...
SELECT COUNT(*) AS count from models WHERE brand_id=(SELECT brand_id FROM brands WHERE name='Ford'); 
...
SELECT m.year, m.name, m.model_id, a.name FROM models AS m LEFT JOIN awards AS a ON m.model_id=a.winner_id WHERE m.year >= 1964;
...
SELECT b.name, m.name, m.year, b.discontinued, (b.discontinued - m.year) AS years_until_brand_discontinued FROM models AS m LEFT JOIN brands AS b USING(brand_id) WHERE b.discontinued IS NOT NULL ORDER BY b.name, m.name, m.year;
...
SELECT b.name, b.founded FROM brands AS b LEFT JOIN models AS m USING(brand_id) WHERE m.model_id IS NULL;
...
SELECT b.brand_id FROM brands AS b JOIN models AS m USING(brand_id) GROUP BY b.brand_id HAVING COUNT(*) > 5;
...
SELECT m.name, m.year FROM models AS m WHERE m.year IN (SELECT b.founded FROM brands AS b);